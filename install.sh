...
# Open firewall port 6000 if UFW is installed
if [ -x "$(command -v ufw)" ]; then
    ufw allow 6000/tcp comment 'LVM Panel Web Interface' >/dev/null 2>&1 || true
fi
...
echo -e "${GREEN}${BOLD}======================================================================${NC}"
echo ""
echo -e "  ${BOLD}Panel URL:${NC}        ${CYAN}http://${VPS_IP}:6000${NC}"
echo -e "  ${BOLD}Admin Email:${NC}      ${GREEN}${ADMIN_EMAIL}${NC}"
echo -e "  ${BOLD}Admin Password:${NC}   ${GREEN}${ADMIN_PASSWORD}${NC}"
echo -e "  ${BOLD}Panel Port:${NC}       6000"
echo -e "  ${BOLD}Service Status:${NC}   systemctl status lvm-panel"
echo ""
echo -e "  ${YELLOW}Notice:${NC} Please open port 6000 in your cloud provider security group"
